...
export async function createProgressBar() {
  ...
  const [setMetadata, activeSet, owned] = await Promise.all([
    get.getSetMetadata() as Promise<types.tcgSet[]>,
    get.getActiveSet(),
    get.getOwnedDb(),
  ]);
  const activeSetData = setMetadata.find((item) => item.id === activeSet);
  const max = (activeSetData?.total ?? 1) as number;
 ...
 */
function countUniqueCardIds(
  setId: string,
  setData: { card_id: string; pulled_date: string }[]
) {
